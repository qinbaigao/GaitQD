...
from .network import TripletLoss, GaitU
...
class Model:
    def __init__(self,
        ...
        self.encoder = GaitU(self.hidden_dim, self.train_pid_num).float()
        ...
        self.view_loss = nn.DataParallel(self.view_loss)
        self.id_loss.cuda()
        self.encoder.cuda()
    ...
    def fit(self):
        ...
        for seq, view, seq_type, label, batch_frame in train_loader:
            ...
            feature, fea_class, angle_probf, angle_probc = self.encoder(*seq, True)

            view_lossf=self.view_loss(angle_probf,view)
            view_lossc=self.view_loss(angle_probc,view)

            fea_class = fea_class.permute(1,0,2).contiguous()

            target_label = [train_label_set.index(l) for l in label]
            target_label = self.np2var(np.array(target_label)).long()

            triplet_feature = feature.permute(1, 0, 2).contiguous()
            p, n, c = fea_class.size()
            class_label = target_label.unsqueeze(0).repeat(fea_class.size(0), 1).view(p*n)
            triplet_label = target_label.unsqueeze(0).repeat(triplet_feature.size(0), 1)
            
            fea_class = fea_class.view(p*n, c)

            id_loss = self.id_loss(fea_class, class_label)
            ...
            loss = loss + id_loss.mean() + 0.5*view_lossf.mean() + 0.5*view_lossc.mean()
    ...
    def transform(self, flag, batch_size=1):
        ...
        for i, x in enumerate(data_loader):
            ...
            if batch_frame is not None:
                batch_frame = self.np2var(batch_frame).int()
            with torch.no_grad():
                feature, _, _, _ = self.encoder(*seq, False)

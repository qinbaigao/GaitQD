...
class TripletLoss_DRPL(nn.Module):
    def __init__(self, batch_size, hard_or_full, margin):
        super(TripletLoss_DRPL, self).__init__()
        self.batch_size = batch_size
        self.margin = margin

    def forward(self, feature, label, iter, total_iter):
        # feature: [n, m, d], label: [n, m]
        n, m, d = feature.size()
        if str(version) != "1.0.1":
            hp_mask = (label.unsqueeze(1) == label.unsqueeze(2)).bool().view(-1)
        ...
        deertat = 0.1 + 0.7*(1+math.cos(math.pi*iter/(2*total_iter)))

        hp_weight = F.softmax(torch.exp(full_hp_dist/deertat), dim = 2)
        hn_weight = F.softmax(torch.exp(0 - full_hn_dist/deertat), dim = 3)
        full_hp_dist *= hp_weight
        full_hn_dist *= hn_weight
